"""Calcul des statistiques de cote a partir des annonces (par modele)."""
...
def _stats(prices: List[int], mileages: List[int]) -> dict:
    ...
    if not prices:
        return {
            "count": 0, "avg_price": None, "median_price": None,
            "min_price": None, "max_price": None, "avg_mileage": None,
        }
    return {
        "count": len(prices),
...
def build_market(listings: Iterable[Listing], model: Model) -> dict:
    """Statistiques globales / par version / par millesime, calees sur le modele."""
    listings = list(listings)

    overall = _stats(
        [l.price for l in listings], [l.mileage for l in listings]
    )
